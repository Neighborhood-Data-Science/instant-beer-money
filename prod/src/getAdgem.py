...
def start_driver_and_open_adgem(url):
    """
...
def main(args):
    driver = start_driver_and_open_adgem(args.get('ADGEM'))
    return {
...
#     adgem_dict = {'offer_title': [], 'offer_description': [],
...
#                                             "//span[@class = 'tw-text-sm satoshi-700 tw-text-[#2D2D2D] \
#                                             tw-leading-5 tw-break-words tw-w-full']")))
#         # Explicit Wait
#         time.sleep(1)
#         # Get sorting buttons on page
#         buttons = driver.find_elements(
#             By.XPATH, "//div[@class = 'tw-relative tw-inline-block text-left tw-px-2 tw-w-full']")
#         # Click this button to open device type menu
#         buttons[-1].click()
#         # Get device sorting options on page
#         options = driver.find_elements(
#             By.XPATH, "//a[@class = 'tw-block tw-px-4 tw-py-2 tw-text-lg tw-text-gray-700 \
...
#             By.XPATH, "//span[@class = 'tw-text-sm satoshi-700 tw-text-[#2D2D2D] \
#             tw-leading-5 tw-break-words tw-w-full']")
#         descriptions = driver.find_elements(
#             By.XPATH, "//span[@class = 'tw-flex tw-mt-1 tw-text-sm tw-text-[#969696] \
#             satoshi-400 tw-break-words tw-w-full']")
#         amounts = driver.find_elements(
#             By.XPATH, "//div[@class = 'tw-flex tw-items-center tw-justify-center tw-mb-1']")
#         devices = driver.find_elements(
#             By.XPATH, "//span[@class = 'tw-hidden md:tw-flex md:tw-items-center md:tw-justify-center']")
#         for (offer_i, desc_i, amount_i, dev_i) in zip(offers, descriptions, amounts, devices):
#             try:
#                 # Grab the name of the offer
#                 name = offer_i.get_attribute('innerText')
#                 # Grab the description of the offer
#                 description = desc_i.get_attribute('innerText')
#                 # Grab the amount of the offer
#                 amount = amount_i.get_attribute('innerText')
#                 # Grab the device name
#                 device = dev_i.get_attribute('innerHTML')
